...
{
    return [NSString
        stringWithFormat:@"<%@: %p; %@>", NSStringFromClass([self class]), self, [self keyValueAutoDescription]];
}
...
- (NSString*)keyValueAutoDescription
{
    ...
    dispatch_queue_t currentQueue = dispatch_queue_create("autoDescription_queue", DISPATCH_QUEUE_CONCURRENT);
