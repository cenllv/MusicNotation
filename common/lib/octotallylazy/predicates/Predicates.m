...
{
    return [^(id item) {
      return [existing containsObject:item];
    } copy];
}
...
{
    return [^(id item) {
      return [[item description] rangeOfString:toMatch].length > 0;
    } copy];
}
...
{
    return [^(id item) {
      return (BOOL)[item isEqual:comparable];
    } copy];
}
...
+ (PREDICATE)greaterThan:(NSNumber*)comparable
{
    return [^(NSNumber* item) {
      return item.doubleValue > comparable.doubleValue;
    } copy];
}
