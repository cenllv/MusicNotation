...
{
    NSInteger rstart = (start >= 0) ? start : self.length - labs(start);
    NSInteger rend = (end >= 0) ? end : self.length - labs(end);
    if(rstart > rend)
        return nil;
...
- (NSString*)center:(NSInteger)amount with:(NSString*)padString
{
    ...
    return (result.length == amount) ? result
                                     : [NSString stringWithFormat:@"%@%c", result, [padString characterAtIndex:c]];
}
...
- (NSString*)chomp:(NSString*)string
{
    return [self stringByReplacingOccurrencesOfString:string
                                           withString:@""
                                              options:0
                                                range:NSMakeRange(self.length - string.length, string.length)];
}
...
- (NSString*)insert:(NSInteger)index string:(NSString*)string
{
    if(index < 0)
        ...
        index = self.length;
    return
        [NSString stringWithFormat:@"%@%@%@", [self substringToIndex:index], string, [self substringFromIndex:index]];
}
...
- (NSString*)inspect
{
    ...
    NSArray* replaceWith =
        [NSArray arrayWithObjects:@"\\0", @"\\a", @"\\b", @"\\t", @"\\n", @"\\f", @"\\r", @"\\e", nil];
    for(NSUInteger i = 0, count = [toReplace count]; i < count; ++i)
    {
        [result replaceOccurrencesOfString:[toReplace objectAtIndex:i]
                                withString:[replaceWith objectAtIndex:i]
                                   options:0
                                     range:myRange];
    }
...
- (NSInteger)lastIndex:(NSString*)pattern offset:(NSInteger)offset
{
    ...
    if(!error)
    {
        NSTextCheckingResult* result =
            [regex matchesInString:self options:0 range:NSMakeRange(0, self.length - offset)].lastObject;
        return (result.range.length > 0) ? result.range.location : NSNotFound;
    }
    return NSNotFound;
}
...
- (NSString*)multiply:(NSUInteger)times
{
...
- (NSString*)squeeze:(NSString*)pattern
{
    ...
    NSError* error = nil;
    NSRegularExpression* regex = [[NSRegularExpression alloc] initWithPattern:wrapped options:0 error:&error];
    if(error)
        return nil;
...
- (id)objectForKeyedSubscript:(id)key
{
    ...
    {   // if it's a string, assume a regex.
        NSError* error = nil;
